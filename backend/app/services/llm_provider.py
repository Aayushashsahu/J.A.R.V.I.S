...
def unwrap_exception_chain(exc: Exception) -> Exception:
    """
    Walk the exception chain to find the innermost real exception.

    google-api-core wraps errors in ``RetryError[<Future …>]`` after its own
    internal retries.  Tenacity may additionally wrap them.  This helper
    peels back every wrapper until the root ``ClientError`` (or whatever
    originally caused the failure) is found.
    """
    current = exc
    # 1. Follow __cause__ / __context__ links
    while True:
    ...
    # 2. google.api_core.exceptions.RetryError stores a Future in `.reason`
    if hasattr(current, 'reason') and hasattr(current.reason, 'exception'):
        future_exc = current.reason.exception()
        if future_exc is not None:
            # Recurse into the future's own chain
            return unwrap_exception_chain(future_exc)
...
class GeminiProvider(BaseLLMProvider):
    ...
    @retry(wait=wait_exponential(multiplier=1, min=2, max=6), stop=stop_after_attempt(3), reraise=True, before_sleep=before_sleep_log(logger, logging.WARNING))
    def _generate_embeddings_nvidia(self, texts: List[str], input_type: str = "passage") -> List[List[float]]:
        """Generate embeddings using NVIDIA NIM API (OpenAI-compatible).
        ...
        self.logger.info(f"[External API Request] NVIDIA embeddings model: {nvidia_embedding_model}, input_type={input_type}, texts count: {len(texts)}")
        ...
        self.logger.info(f"[External API Response] NVIDIA embeddings success, generated {len(embeddings)} vectors, dim={len(embeddings[0])}")
        return embeddings

    def generate_embeddings(self, texts: List[str], input_type: str = "passage") -> List[List[float]]:
        """Generate embeddings with Gemini primary, NVIDIA fallback.
        ...
        if settings.GEMINI_API_KEY:
            try:
                ...
                root = unwrap_exception_chain(e)
                self.logger.warning(
                    f"Gemini embeddings failed: {type(root).__name__}: {root}. "
                    f"Falling back to NVIDIA."
                )
        # Fallback to NVIDIA
        if self.nvidia_client is not None:
            try:
                return self._generate_embeddings_nvidia(texts, input_type=input_type)
            except Exception as e:
                root = unwrap_exception_chain(e)
                self.logger.error(
                    f"[External API Error] NVIDIA embeddings also failed: "
        ...
        raise RuntimeError(
            "No embedding provider available. Set GEMINI_API_KEY or NVIDIA_API_KEY."
        )
